...
module.exports = async function bundler({
    ...
    workDir = 'output',
    outputDir = 'loader',
    rootPath = path.resolve('../../..'), // goes up from /tao/views/build
    extension = '',
    rootExtension = '',
    bundles = [],
    dependencies = [],
    ...
    const createRJsBundleConfig = async (bundle) => {
        ...
        if(Array.isArray(bundle.exclude)){
            console.log('excluding bundle.exclude modules', bundle.exclude);
        ...
        if (Array.isArray(dependencies)) {
            ...
            dependencies.forEach( async (dependency) => {
                try {
                    // load the requirejs paths of each dependency into the excludes
                    const dependencyPaths = require(`${rootPath}/${dependency}/views/build/grunt/paths.json`);
                    const toExclude = Object.keys(dependencyPaths).map(p => `${p}/**/*`);
                    console.log(`excluding ${dependency} paths.json modules`, toExclude);
                    await excludeModules(toExclude);
                }
                catch (e) {
                }
            });
    ...
    const createGlobalExclude = async () => {
        ...
        const excludeExtension = async targetExtension => {
            console.log('excluding', targetExtension);
