...
module.exports = async function bundler({
    ...
    const createRJsBundleConfig = async (bundle) => {
        ...
        if(loadBundle.length){
            //simply register the dependency in a global variable
            //so other bundled loader have access to it.
            bundleConfig.override = {
                wrap : {
                    start : '',
                    end : `window.bundles = (window.bundles || []).concat(${JSON.stringify(loadBundle)});`
                }
            };
        }
