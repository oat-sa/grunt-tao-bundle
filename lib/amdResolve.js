 ...
 */
module.exports = function amdResolve(
    pattern,
    {
        targetExtension = '',
        dependencies = [],
    ...
    if (!supportedModuleTypes[type]) {
        throw new TypeError(
            `Unsupported module type '${type}', please select one in ${Object.keys(supportedModuleTypes).join(',')}.`
        );
    }
    ...
    const matchingAlias = Object.entries(aliases).find(([alias]) => pattern.startsWith(`${alias}/`));

    //resolve the aliases, only to resolve the modules against the file system
    let aliasedPattern = pattern;
    if (matchingAlias) {
        aliasedPattern = path.join(matchingAlias[1], pattern.substring(matchingAlias[0].length));
    }

    //remove the extension prefix from the pattern :
    //taoQtiTest/runner/**/* becomes runner/**/*
    //because we resolve from the extensionPath
    const filePattern = globPath(aliasedPattern.replace(new RegExp(`^${targetExtension}`, '')));
            ...
            (err, matches) => {
                ...
                const needsPrefix = file => {
                    ...
                    if (Array.isArray(dependencies) && dependencies.length) {
                        if (
                            dependencies.some(dep => file.startsWith(dep)) ||
                ...
                return resolve(
                    matches
                        ...
                        .map(file => {
                            ...
                            //unalias
                            if (matchingAlias) {
                                moduleName = path.join(matchingAlias[0], moduleName.substring(matchingAlias[1].length));
                            }
