 ...
 * @returns {string}
 */
const getSourceMapWithPath = sourceMapObject =>
    `sourceMappingURL=${sourceMapObject.path}/${sourceMapObject.file}.css.map`;
 ...
 */
module.exports = function resolve({ mapBody, codeBody, paths, cssSourceMapPath }) {
    let resolved = null;
    ...
    matchedSourceMaps.forEach(function (matchedSourceMap) {
        ...
        const extractedPaths = new RegExp(
            `css!([\\w|\\-|.|\\/]+)/(${matchedSourceMap.file}\\.css|${matchedSourceMap.file}\\'|${matchedSourceMap.file}\\")`
        ).exec(codeBody);
